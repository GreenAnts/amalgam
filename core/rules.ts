 ...
 */
export function executeFireballAbility(state: GameState, formation: Vector2[], direction: Vector2, pieceDefs: PieceDefinitions, movedPieceCoords?: Vector2): MoveResult {
 ...
 */
function validateMovementRequirement(formation: Vector2[], movedPieceCoords?: Vector2): boolean {
    if (!movedPieceCoords) {
        return false; // No piece moved this turn
    }
    
    // Check if any piece in formation matches the moved piece coordinates
    return formation.some(formationCoords => 
        formationCoords[0] === movedPieceCoords[0] && 
 ...
 */
function validateFireballFormation(state: GameState, formation: Vector2[], pieceDefs: PieceDefinitions): { valid: boolean; reason?: string } {
 ...
 */
function checkFireballAmplification(state: GameState, formation: Vector2[]): boolean {
    const [pos1, pos2] = formation;
    
    // Calculate formation direction
    const direction = [pos2[0] - pos1[0], pos2[1] - pos1[1]] as Vector2;
    
    // Check both ends of the formation for Void piece (following reference lines 190-193)
    const voidPos1 = [pos1[0] - direction[0], pos1[1] - direction[1]] as Vector2;
    const voidPos2 = [pos2[0] + direction[0], pos2[1] + direction[1]] as Vector2;
 ...
 */
function getFireballTargets(state: GameState, formation: Vector2[], direction: Vector2, amplified: boolean): Vector2[] {
 ...
 */
export function getFireballDirections(formation: Vector2[]): Vector2[] {
    const [pos1, pos2] = formation;
    const formationDirection = [pos2[0] - pos1[0], pos2[1] - pos1[1]] as Vector2;
 ...
 * @returns Validation result
 */
export function validateFireballMove(state: GameState, move: Move, pieceDefs: PieceDefinitions, movedPieceCoords?: Vector2): MoveResult {
    ...
    const directionValid = availableDirections.some(
