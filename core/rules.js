...
function validateNexusMove(state, move, pieceDefs) {
    ...
    const isDestinationAdjacentToNexus = nexusFormations.some(formation => formation.pieces.some(nexusCoords => areAdjacent(toCoords, nexusCoords)));
...
function validatePortalLineMove(state, move, pieceDefs) {
    ...
    const isDirectlyConnected = connections.some(conn => conn[0] === toCoords[0] && conn[1] === toCoords[1]);
 ...
 * @returns Validation result
 */
function validatePortalStandardMove(state, move) {
    const { fromCoords, toCoords } = move;
    if (!fromCoords || !toCoords) {
        return { ok: false, reason: 'Missing from or to coordinates' };
    }
 ...
 * @returns Validation result
 */
function validatePortalPhasingMove(state, move) {
    const { fromCoords, toCoords } = move;
    if (!fromCoords || !toCoords) {
        return { ok: false, reason: 'Missing from or to coordinates' };
    }
...
export function applyMove(state, move, pieceDefs) {
    ...
    switch (move.type) {
        case 'place':
            ...
            const graphics = pieceDef.graphics || {
        ...
        case 'portal_swap':
            ...
            const fromPiece = newState.board.intersections.find(int => int.coords[0] === move.fromCoords[0] && int.coords[1] === move.fromCoords[1]);
            const toPiece = newState.board.intersections.find(int => int.coords[0] === move.toCoords[0] && int.coords[1] === move.toCoords[1]);
...
function performAutomaticAttack(state, coords, pieceDefs) {
    ...
    if (pieceDef.type === 'Portal') {
        ...
        for (const connection of goldenConnections) {
            ...
            if (!connectedIntersection || !connectedIntersection.piece)
                continue;
        ...
        for (const adjacent of adjacentIntersections) {
            ...
            if (pieceDef.player === adjacentPieceDef.player)
                continue;
...
export function checkWin(state, pieceDefs) {
    ...
    const circlesPieces = Object.values(state.pieces).filter(p => p.player === 'circles' &&
        !p.isPrePlaced &&
        ['Ruby', 'Pearl', 'Amber', 'Jade'].includes(p.type));
    const squaresPieces = Object.values(state.pieces).filter(p => p.player === 'squares' &&
        !p.isPrePlaced &&
        ['Ruby', 'Pearl', 'Amber', 'Jade'].includes(p.type));
...
function getValidSetupPositions(state, playerId, pieceDefs) {
    const startingArea = playerId === 'circles' ?
        pieceDefs.board_data.starting_areas.circles_starting_area.positions :
        pieceDefs.board_data.starting_areas.squares_starting_area.positions;
    return startingArea.filter(position => isEmptyIntersection(state.board, position));
}
/**
    ...
    if (nexusFormations.length > 0) {
        ...
        if (isAdjacentToNexus) {
            ...
            for (const formation of nexusFormations) {
                ...
                const isAdjacentToThisFormation = formation.pieces.some(nexusCoords => areAdjacent(piece.coords, nexusCoords));
