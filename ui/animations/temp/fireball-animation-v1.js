...
export class FireballAnimation {
    constructor(config) {
        ...
        this.flowField = []; // Flow field for natural smoke movement
    ...
    generateCurvePath(startX, startY, endX, endY) {
    ...
    updateEmissionPhase(particle, deltaTime) {
    ...
    updateFlowingPhase(particle, deltaTime) {
    ...
    updateGatheringPhase(particle, deltaTime) {
        ...
        particle.spearAngle = Math.atan2(this.config.targetPosition.y - forwardPiece.y, this.config.targetPosition.x - forwardPiece.x);
        // Add to trail for smoke effect
        this.addToTrail(particle);
    ...
    updateSpearPhase(particle, deltaTime) {
        ...
        particle.targetDistance = Math.sqrt(Math.pow(this.config.targetPosition.x - particle.x, 2) +
