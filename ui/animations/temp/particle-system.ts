...
export interface ParticleConfig {
  ...
  direction: 'random' | 'radial' | 'linear' | 'spiral';
  bounds: { width: number; height: number };
}

export type ParticlePresetName = 'fireball' | 'tidalWave' | 'sap' | 'launch';
...
export class Particle {
  ...
  private getInitialVelocity(direction: string, speed: number): Vector2D {
    switch (direction) {
      case 'radial':
...
export class ParticleSystem {
  ...
  addForce(centerX: number, centerY: number, strength: number, type: 'attract' | 'repel'): void {
    for (const particle of this.particles) {
...
export const ParticlePresets: Record<ParticlePresetName, ParticleConfig> = {
  fireball: {
    count: 200,
    speed: 2,
    size: 3,
