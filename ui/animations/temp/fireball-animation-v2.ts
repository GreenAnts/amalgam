...
// Fireball Animation V2 for Amalgam Game (unique blend of V1 and V3)
// - Anticipation charge around rubies
// - Arced flight with light flow-field influence
// - Ember trail particles (lightweight)
// - Impact shockwave and modest fragments

export interface FireballV2Config {
...
type Phase = 'charge' | 'launch' | 'impact' | 'fade';
...
interface Fragment {
  ...
  points: Array<{ x: number; y: number }>; // local-space polygon points
  fill: string;
}

export class FireballV2Animation {
  private config: FireballV2Config;
  ...
  private shockwave: { r: number; alpha: number } | null = null;
  private destroyed = false;

  // light flow influence grid (coarser and cheaper than V3)
  private flowAngles: number[][] = [];

  constructor(config: FireballV2Config) {
    this.config = config;
  ...
  private computeMidpoint(a: { x: number; y: number }, b: { x: number; y: number }) {
    return { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 };
  }

  private getForwardMostPiece(pieces: Array<{ x: number; y: number }>, target: { x: number; y: number }) {
  ...
  private computeArcControlPoint(start: { x: number; y: number }, end: { x: number; y: number }) {
  ...
  private flowAt(x: number, y: number): number {
  ...
  private squigglePoint(t: number): { x: number; y: number } {
  ...
  private spawnChargeEmbers(dt: number): void {
  ...
  private spawnTrailEmbers(pos: { x: number; y: number }, dt: number): void {
  ...
  private createImpactEffects(): void {
    if (this.destroyed) return;
    this.destroyed = true;
    if (this.config.onTargetDestroyed) this.config.onTargetDestroyed();
    // shockwave ring
    ...
    const quads: Array<{ pts: Array<{ x: number; y: number }>; color: string; dir: { x: number; y: number } }> = [
  ...
  private getShardBaseSize(): number {
    // Derive shard sizing from graphics configuration per target type
    try {
      const cfg = (graphicsConfig as any).getConfig ? (graphicsConfig as any).getConfig() : null;
      const pcs = cfg ? cfg.pieces : undefined;
      const tt = (this.config.targetType || 'Amalgam').toLowerCase();
      let base: number | undefined;
      if (tt === 'amalgam') base = pcs?.amalgam?.size;
      else if (tt === 'portal') base = pcs?.portal?.size;
      else if (tt === 'void') base = pcs?.void?.size;
      else if (tt === 'ruby') base = pcs?.gems?.ruby?.size;
      else if (tt === 'pearl') base = pcs?.gems?.pearl?.size;
      else if (tt === 'amber') base = pcs?.gems?.amber?.size;
      else if (tt === 'jade') base = pcs?.gems?.jade?.size;
      if (typeof base !== 'number') base = 14;
      const scaled = (graphicsConfig as any).getScaledSize ? (graphicsConfig as any).getScaledSize(base) : base;
  ...
  private getTargetQuadrantColors(): string[] {
    try {
      const cfg = (graphicsConfig as any).getConfig ? (graphicsConfig as any).getConfig() : null;
      const pcs = cfg ? cfg.pieces : undefined;
      ...
      if (tt === 'Ruby' && pcs?.gems?.ruby?.color) return [pcs.gems.ruby.color, pcs.gems.ruby.color, pcs.gems.ruby.color, pcs.gems.ruby.color];
      if (tt === 'Pearl' && pcs?.gems?.pearl?.color) return [pcs.gems.pearl.color, pcs.gems.pearl.color, pcs.gems.pearl.color, pcs.gems.pearl.color];
      if (tt === 'Amber' && pcs?.gems?.amber?.color) return [pcs.gems.amber.color, pcs.gems.amber.color, pcs.gems.amber.color, pcs.gems.amber.color];
      if (tt === 'Jade' && pcs?.gems?.jade?.color) return [pcs.gems.jade.color, pcs.gems.jade.color, pcs.gems.jade.color, pcs.gems.jade.color];
  ...
  private updateFragments(dt: number): void {
    ...
    if (this.shockwave) {
      ...
      if (this.shockwave.alpha < 0.06) this.shockwave = null;
    }
  }
  ...
  isComplete(): boolean { return !this.isActive; }
}

export class FireballAnimationManager {
  private animations: FireballV2Animation[] = [];
  ...
  update(deltaTime: number): void {
    for (let i = this.animations.length - 1; i >= 0; i--) {
      ...
      a.update(deltaTime);
      if (a.isComplete()) this.animations.splice(i, 1);
    }
  }

  draw(ctx: CanvasRenderingContext2D): void {
    for (const a of this.animations) a.draw(ctx);
  }

  getActiveAnimationCount(): number { return this.animations.length; }
  clearAll(): void { this.animations = []; }
}
