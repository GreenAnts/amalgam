...
import { getFireballDirections } from './core/rules.js';
/**
...
export class AmalgamGame {
    ...
    async initializeUI() {
        ...
        this.undoIconButton = document.getElementById('undo-icon');
        this.endTurnIconButton = document.getElementById('end-turn-icon');
    ...
    setupEventListeners() {
        ...
        const hookTooltip = (btnId) => {
            ...
            btn.addEventListener('mouseenter', () => {
                ...
                const tabRect = this.portalSwapButton?.closest('.ability-tab')?.getBoundingClientRect();
        ...
        this.fireballButton?.addEventListener('click', () => {
            if (this.fireballButton?.disabled)
                return;
     ...
     */
    async startFireballAnimation(formation, direction) {
    ...
    handleEndTurn() {
        ...
        if (newState && newState.gamePhase === 'gameplay' && currentPlayer && currentPlayer.type !== 'human') {
            ...
            if (this.gameManager && 'handleAITurnDuringGameplay' in this.gameManager) {
                this.gameManager.handleAITurnDuringGameplay();
            }
        }
     ...
     */
    handleFireballFromAbility(ability) {
    ...
    renderPieceSelectionPanel(state) {
        ...
        if (!state || state.gamePhase !== 'setup') {
            ...
            this.pieceSelectionPanel.style.display = 'none';
            return;
        }
        ...
        if (isAIPlayer) {
            ...
            this.pieceSelectionPanel.style.display = 'flex';
            this.pieceSelectionPanel.innerHTML = `
        ...
        Array.from(this.pieceSelectionPanel.querySelectorAll('.piece-select-btn')).forEach((btn) => {
            ...
            try {
                ...
                if (ctx) {
                    ...
                    renderPiece(tempContext, piece, [0, 0]);
                }
            }
            catch (_) {
    ...
    updateGameInfo(state) {
        ...
        if (listLeft && listRight && this.pieceDefs) {
            ...
            Object.values(allDefs).forEach((def) => allTypesSet.add(def.type));
            // Count pieces on the board by type and player
            const countsOnBoardByPlayer = {
                circles: {},
                squares: {}
            };
            Object.values(state.pieces).forEach((p) => {
                const bucket = countsOnBoardByPlayer[p.player];
            ...
            const makeRows = (player) => types.map((t) => {
        ...
        for (let i = this.lastLoggedMoveIndex; i < history.length; i++) {
            ...
            if (m.ability && m.ability.type) {
                ...
                const player = m.playerId || m.meta?.movedBy || piece?.player;
    ...
    appendMoveLog(entry) {
        ...
        const dotStyle = type === 'Amalgam' && colors.length >= 4
            ...
            : colors[0];
        icon.style.background = dotStyle;
        // Compose text
        if (entry.ability) {
     ...
     */
    findFireballFormationForPiece(state, piece) {
        ...
        const playerPieces = Object.values(state.pieces).filter(p => p.player === piece.player && validTypes.includes(p.type));
     ...
     */
    showFireballDirections(formation, directions) {
     ...
     */
    getFireballDirectionFromClick(clickCoords, formation, availableDirections) {
        ...
        for (const direction of availableDirections) {
            ...
            const distance = Math.sqrt(Math.pow(clickCoords[0] - endpoint[0], 2) +
     ...
     */
    executeFireballInDirection(direction) {
